...
if(isset($_POST['login'])) {
    ...
    if(empty($name) || empty($password)) {
        ...
        // Query based on role
        if($role == 'user') {
            $sql = "SELECT * FROM accounts WHERE role='user' AND name='$name'";
        } else if($role == 'admin') {
            $sql = "SELECT * FROM accounts WHERE role='admin' AND name='$name'";
        }
    ...
    <style>
        * {
        ...
        .role-option:hover {
            ...
            box-shadow: 0 8px 20px rgba(81, 134, 63, 0.15);
        }
        
        .role-option.selected {
            border-color: #511600;
            background: linear-gradient(135deg, #511600 0%, #51863f 100%);
            color: white;
            box-shadow: 0 8px 20px rgba(81, 134, 63, 0.25);
        }
        ...
        input[type="password"]:focus {
            ...
            border-color: #51863f;
            background: #FFF;
            box-shadow: 0 0 0 4px rgba(81, 134, 63, 0.1);
        }
        ...
        .submit-btn:hover {
            transform: translateY(-3px);
            box-shadow: 0 12px 25px rgba(81, 134, 63, 0.3);
        }
